import type { EnrichedMatch, Stream, Sport, Movie, Genre, MediaType } from './types';
...
function normaliseMatch(m: any): EnrichedMatch {
  const status = m.status === 'inprogress' ? 'live'
...
const TMDB_KEY = import.meta.env.VITE_TMDB_KEY ?? '8265bd1679663a7ea12ac168da84d2e8';
...
export function getEmbedSources(tmdbId: number, type: MediaType, season?: number, episode?: number): Stream[] {
...
export async function fetchTrending(type: MediaType = 'movie'): Promise<Movie[]> {
  const data = await tmdbGet(`/trending/${type}/week`);
  return (data.results ?? []).map((m: any) => normaliseMovie(m, type));
}

export async function fetchPopular(type: MediaType = 'movie'): Promise<Movie[]> {
  const data = await tmdbGet(`/${type}/popular`);
  return (data.results ?? []).map((m: any) => normaliseMovie(m, type));
}

export async function fetchTopRated(type: MediaType = 'movie'): Promise<Movie[]> {
...
export async function fetchByGenre(type: MediaType, genreId: number, page = 1): Promise<Movie[]> {
  const data = await tmdbGet(`/discover/${type}`, { with_genres: String(genreId), page: String(page), sort_by: 'popularity.desc' });
  return (data.results ?? []).map((m: any) => normaliseMovie(m, type));
}
...
export async function fetchMovieDetails(tmdbId: number, type: MediaType) {
  ...
  const seasons: { season_number: number; episode_count: number }[] = data.seasons ?? [];
  return {
...
export async function fetchSimilar(tmdbId: number, type: MediaType): Promise<Movie[]> {
  const data = await tmdbGet(`/${type}/${tmdbId}/similar`);
  return (data.results ?? []).slice(0, 12).map((m: any) => normaliseMovie(m, type));
}
